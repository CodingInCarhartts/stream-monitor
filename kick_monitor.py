            ...
            async with websockets.connect(
                ...
                async for message in websocket:
                    try:
                        ...
                        if data.get('event') == 'App\\Events\\ChatMessageEvent':
                            ...
                            try:
                                ...
                                print(f'Message processing timed out for {channel_name}, skipping to prevent buffer accumulation')
                                continue
                    ...
                    if time.time() - last_activity > 300:  # 5 minutes
                        print(f'No activity for 5 minutes on {channel_name}, forcing reconnection...')
                        break

        except websockets.exceptions.ConnectionClosed:
            print(f'WebSocket connection closed for {channel_name}, reconnecting in {backoff_delay:.1f} seconds...')
            await asyncio.sleep(backoff_delay)
            backoff_delay = min(backoff_delay * 2, max_backoff)  # Exponential backoff
        except Exception as e:
            print(f'Error in WebSocket connection for {channel_name}: {e}, reconnecting in {backoff_delay:.1f} seconds...')
            await asyncio.sleep(backoff_delay)
            backoff_delay = min(backoff_delay * 2, max_backoff)  # Exponential backoff
        else:
